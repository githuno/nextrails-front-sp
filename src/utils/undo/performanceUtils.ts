import { OperationRecord, UndoStackState, UndoDebugger } from './types';
...
export function withPerformanceTracking<R>(
  ...
  try {
    ...
    const duration = perfMonitorInstance 
      ? perfMonitorInstance.endTimer(startTime)
      : undefined;
...
export function createDebugger(enabled: boolean): UndoDebugger {
  ...
  return {
    ...
    warn: (message: string, ...args: any[]) => 
      console.warn(`[UndoStack] ${message}`, ...args),
    error: (message: string, ...args: any[]) => 
      console.error(`[UndoStack] ${message}`, ...args),
  };
}