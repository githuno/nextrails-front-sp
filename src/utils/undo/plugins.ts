...
import { UndoStackPlugin, UndoableAction, MemoryUsage } from './types';
...
export function createHistoryPersistencePlugin<T>(
  ...
  const saveData = (state: T) => {
    try {
      ...
      // 状態のハッシュを計算（簡易版）
      const currentHash = hashAlgorithm === 'simple'
        ? objUt.computeStateHash(state)
      ...
      if (immutable) {
        ...
        // 選択的クローン
        lastSavedState = objUt.selectiveDeepClone(
...
export function createDebugPlugin<T>(
  ...
  // 各種メトリクス
  let events: { type: EventType; label?: string; timestamp: number; details?: any }[] = [];
  let actionCount = 0;
  let undoCount = 0;
  let redoCount = 0;
  ...
  const calculateMetrics = () => {
    ...
    const avgActionsPerBatch = batchActions.length > 0
      ? batchActions.reduce((sum, item) => sum + (item.details?.batchSize || 1), 0) / batchActions.length
      : 0;

    // メモリ使用量の計算
    const memoryUsage = memoryStats.samples > 0 
      ? {
  ...
  const recordEvent = (type: EventType, label?: string, details?: any) => {
    ...
    if (enableConsole) {
      ...
      console.log(
        `[UndoDebug] ${type}${labelInfo}${detailsInfo} @ ${new Date().toISOString()}`
      );
    }
  };
