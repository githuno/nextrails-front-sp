import { UndoableAction, MemoryUsage, UndoStackOptions } from './types';
import { objUt } from '../objectUtils';
 ...
 * スタック全体のメモリ使用量を推定
 */
export function estimateTotalMemoryUsage<T>(
  ...
  return {
    ...
    averageActionSize: (past.length + future.length > 0) 
      ? totalSize / (past.length + future.length) 
      : 0,
    largestAction
  };
}
 ...
 */
export function trimHistoryBasedOnMemory<T>(
  past: UndoableAction<T>[],
 ...
 * 大きなアクションデータを分離して保存
 */
export function optimizeActionForStorage<T>(
...
export function enhancedGarbageCollection<T>(
  ...
  future: UndoableAction<T>[],
  options: UndoStackOptions
): { past: UndoableAction<T>[]; future: UndoableAction<T>[]; freedBytes: number } {
  // 現在のメモリ使用量を計算
  ...
  if (future.length > 0) {
    optimizedFuture = future.map(action => 
  ...
  if (freedBytes < 1024 * 100 && past.length > 100) { // 100KB未満しか解放できない場合
    // より古いアクションをさらに削減
...
export function getAdjustedGCInterval(
  pastLength: number,
  baseInterval: number
): number {
  if (pastLength > 200) {
