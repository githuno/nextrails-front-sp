...
import { JobResult, useWebWorker } from "@/hooks/useWorker"
...
export default function AdvancedJobWorkerDemo() {
  ...
  useEffect(() => {
    log(`Workerライフサイクル設定を変更: ${terminateAfterJob ? "ジョブ後に終了" : "ジョブ間で再利用"}`)
  }, [terminateAfterJob])
      ...
      if (worker) {
        ...
        try {
          ...
          if (response?.workerId) {
            ...
            setLastWorkerId(response.workerId)
  ...
  useEffect(() => {
    ...
    const handleWorkerTerminated = (event: Event) => {
      ...
      if (workerCreatedRef.current) {
        ...
        log(`Worker終了イベントを受信しましたが、既に処理済みです (終了元: ${terminationSource})`)
      }
    }
    ...
    if (workerLifecycle.created > 0 && workerLifecycle.terminated > 0) {
      const checkNewWorkerId = async () => {
        ...
        try {
          ...
          if (newWorkerId && lastWorkerIdRef.current && newWorkerId !== lastWorkerIdRef.current) {
            setWorkerLifecycle((prev) => ({
              ...prev,
  ...
  }, [workerLifecycle.created, workerLifecycle.terminated, getWorkerRef, sendDirectMessage])
  ...
  - 期待値 (作成 = 終了 + 活性): ${expected} = ${actual} (${expected === actual ? "一致" : "不一致"})
  `)
  ...
  return (
    <div className="mx-auto max-w-4xl p-10">
      <h1 className="mb-2 text-2xl font-bold">Web Worker デモ</h1>
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div className="rounded-lg border p-4">
          ...
          <p className="mb-4 text-xs text-gray-600">※このデモでは意図的に計算を遅くしています（進捗バー確認用）</p>
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="mb-4">
              <label className="mb-2 block">{calculationType === "delayDemo" ? "ステップ数:" : "数値:"}</label>
              <input
                type="text"
                value={input}
          ...
          <div className="mb-4 flex justify-end space-x-2">
            <button
              ...
              disabled={!isRunning}
              className="flex-grow rounded bg-red-600 px-4 py-2 text-white disabled:bg-gray-400"
            >
              ジョブを中止
            </button>
            <button
          ...
          {isRunning && (
            <div className="mb-4">
              <div className="mb-2 h-2.5 w-full rounded-full bg-gray-200">
                <div
                  className="h-2.5 rounded-full bg-blue-600 transition-all duration-200 ease-in-out"
          ...
          <div className="mb-4 rounded bg-gray-100 p-2">
            ...
            <div className="mt-1 flex justify-between text-xs">
              ...
              <span className="font-mono">
                {workerStatus === "アクティブ" ? lastWorkerId || "取得中..." : "未初期化"}
              </span>
            </div>
            <div className="mt-1 flex justify-between text-xs">
              <span>状態:</span>
          ...
          <div className="mb-4 rounded border border-gray-200 bg-gray-50 p-3">
            <h3 className="mb-2 text-sm font-semibold">Workerライフサイクル設定</h3>
            ...
            <p className="text-xs text-gray-800">※ 作成回数 = 終了回数 + アクティブなWorker数</p>
            ...
            <button
              ...
              className="mt-2 rounded bg-purple-200 px-3 py-1 text-xs text-purple-800"
            >
              カウント整合性確認
            </button>
          </div>
        ...
        {/* 上級者向け制御パネル */}
        <div className="rounded-lg border p-4">
          <h2 className="mb-3 text-xl font-semibold">上級者向け制御</h2>

          <div className="mb-4">
            <div className="mb-3 grid grid-cols-3 items-center">
              <div className="flex flex-col text-center">
                <p className="text-xs">Worker状態</p>
                <p className={workerStatus === "アクティブ" ? "font-bold text-green-600" : "font-bold text-red-600"}>
                  {workerStatus}
                </p>
              </div>
            ...
            <div className="mb-4 flex space-x-2">
              <button
                ...
                className="rounded bg-green-600 px-3 py-2 text-sm text-white disabled:bg-gray-400"
              >
                Worker初期化
              </button>
              ...
              <button
                ...
                disabled={workerStatus !== "アクティブ"}
                className="rounded bg-red-600 px-3 py-2 text-sm text-white disabled:bg-gray-400"
              >
                Worker終了
              </button>

              <button onClick={handleAbortAllJobs} className="rounded bg-yellow-600 px-3 py-2 text-sm text-white">
                全ジョブ中止
              </button>
            </div>
          </div>
          ...
          <div className="mb-4">
            <label className="mb-2 block">カスタムメッセージ:</label>
            <textarea
              value={customMessage}
            ...
            <p className="mt-1 text-xs text-gray-500">※Workerが存在しない場合は自動的に初期化されます</p>
            <p className="mt-1 text-xs text-gray-500">
      ...
      <div className="mb-6 rounded-lg border p-4">
        <h2 className="mb-3 text-xl font-semibold">実行ログ</h2>
        <div className="h-64 overflow-y-auto rounded bg-gray-100 p-3 font-mono text-sm">
          {output.length === 0 ? (
        ...
        <button onClick={() => setOutput([])} className="mt-2 rounded bg-gray-200 px-3 py-1 text-sm text-gray-800">
          ログをクリア
        </button>
      </div>
      ...
      {/* 結果履歴パネル */}
      <div className="mb-6 rounded-lg border p-4">
        <h2 className="mb-3 text-xl font-semibold">結果履歴</h2>
        ...
        {resultHistory.length === 0 ? (
          ...
          <div className="max-h-64 overflow-y-auto">
              ...
              <tbody>
                {resultHistory.map((result, index) => (
                  <tr key={index} className={index % 2 === 0 ? "bg-gray-50" : ""}>
                    <td className="p-2">{result.jobId?.split("_").slice(0, 2).join("_")}</td>
                    <td className="p-2">{result.status}</td>
                    <td className="max-w-md truncate p-2">{result.data !== null ? String(result.data) : "N/A"}</td>
            ...
            <button
              ...
              className="mt-2 rounded bg-gray-200 px-3 py-1 text-sm text-gray-800"
            >
              履歴をクリア
            </button>
          </div>
      ...
      <div className="mt-4 border-t pt-4 text-xs text-gray-500">
        <p>このデモでは、useWebWorkerフックの全ての機能を試すことができます：</p>
      ...
      <div className="mt-6">
        ...
        <p className="mt-1 text-xs text-gray-500">
          ※シンプルなデモでは、useWebWorkerフックの基本機能のみを使用しています。
          <br />
          上級者向けオプションは使用できません。
        </p>
      </div>
