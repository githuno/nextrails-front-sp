...
const DrawingPage: React.FC = () => {
  ...
  }, [drawing, currentStroke, history, canUndo])
  ...
  useEffect(() => {
    renderCanvas()
  }, [drawing, currentStroke, renderCanvas])

  // マウス/タッチイベントからの座標取得 - 修正版
  const getCoordinates = (e: React.MouseEvent | React.TouchEvent | TouchEvent | MouseEvent): Point => {
    const canvas = canvasRef.current
  ...
  return (
    <div className="flex flex-col p-10">
      ...
      <div className="mb-6 rounded-lg border border-blue-200 bg-blue-50 p-4">
        <h2 className="mb-2 text-lg font-semibold">useUndoフックについて</h2>
        ...
        <ul className="ml-5 list-disc text-sm">
          ...
          <li>
            Undo/Redoボタンまたは
            <kbd className="rounded bg-gray-200 px-1 py-0.5">Ctrl+Z</kbd>/
            <kbd className="rounded bg-gray-200 px-1 py-0.5">Ctrl+Y</kbd>
            で操作を元に戻せます
          </li>
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex space-x-2">
          <button
            onClick={undo}
            disabled={!canUndo}
            className={`rounded px-3 py-2 ${canUndo ? "bg-blue-500 text-white" : "bg-gray-300 text-gray-500"}`}
          >
            Undo
          </button>
          ...
          <button
            onClick={redo}
            disabled={!canRedo}
            className={`rounded px-3 py-2 ${canRedo ? "bg-blue-500 text-white" : "bg-gray-300 text-gray-500"}`}
          >
            Redo
          </button>

          <button onClick={clearCanvas} className="rounded bg-red-500 px-3 py-2 text-white">
            クリア
          </button>
        </div>
      </div>
      ...
      {/* キャンバスエリアに固定の高さを設定 */}
      <div
        className="relative flex-grow overflow-hidden rounded-lg border border-gray-300"
      ...
      <div className="mt-4 rounded-md border border-gray-200 bg-gray-50 p-3">
        <h3 className="mb-2 text-sm font-medium">線の設定:</h3>
            ...
            {["#000000", "#FF0000", "#00FF00", "#0000FF", "#FFFF00", "#FF00FF", "#00FFFF"].map((color) => (
              <div
                ...
                className={`h-8 w-8 cursor-pointer rounded-full border ${
                  drawing.currentColor === color ? "border-2 border-gray-800" : "border-gray-300"
                }`}
      ...
      {/* 高度な履歴操作セクション */}
      <div className="mt-4 rounded-md border border-gray-200 bg-gray-50 p-3">
        <h3 className="mb-2 text-sm font-medium">高度な操作:</h3>
        <div className="flex flex-wrap gap-2">
          {/* 状態の保存と復元 */}
          <div className="flex gap-2">
            <button onClick={saveCurrentState} className="rounded bg-green-500 px-3 py-1 text-sm text-white">
              保存
            </button>
            <button onClick={loadSavedState} className="rounded bg-amber-500 px-3 py-1 text-sm text-white">
              読み込み
            </button>
          </div>
          ...
          <button
            ...
            disabled={history.filter((h) => !h.label.includes("silent") && h.label !== "初期状態").length < 2}
            className="rounded bg-indigo-500 px-2 py-1 text-xs text-white disabled:bg-gray-300"
          >
            2つ前の表示状態に戻る
          </button>
          <button
            ...
            className="rounded bg-purple-500 px-2 py-1 text-xs text-white disabled:bg-gray-300"
          >
            初期状態まで戻る
          </button>
        </div>
      ...
      <div className="mt-4 border-t pt-2" style={{ flex: "0 0 auto" }}>
        <div className="mb-2 flex items-center justify-between">
          <h3 className="text-sm font-medium">履歴 ({history.length} アクション):</h3>
          <button onClick={clear} className="rounded bg-gray-500 px-3 py-1 text-xs text-white">
            履歴削除
          </button>
        </div>
        ...
        <div className="max-h-[150px] overflow-y-auto pr-2">
          <div className="flex flex-col gap-2">
            {history.map((item, index) => (
              <div key={index} className="rounded border border-gray-300 bg-gray-100 px-2 py-1 text-xs">
      ...
      <div className="mt-4 border-t pt-2">
        <button onClick={() => setShowDetails(!showDetails)} className="text-xs text-blue-600 underline">
        ...
        {showDetails && (
          <div className="mt-2 rounded bg-gray-50 p-2 text-xs">
            ...
            <p>推定サイズ: {(memoryUsage.estimatedBytes / 1024).toFixed(2)} KB</p>
            <p>平均アクションサイズ: {(memoryUsage.averageActionSize / 1024).toFixed(2)} KB</p>
