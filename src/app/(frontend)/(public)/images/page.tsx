...
export default function Page() {
  ...
  const fetchItems = useCallback(async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE}/get_objects`, {
        method: "POST",
        mode: "cors",
        // credentials: "include",
        headers: {
          ...
          <ListGroupItem key={item.id}>
            <div className="flex items-center gap-4">
              {item.main_image.endsWith(".mp4") ? (
                ...
                <Image src={item.main_image} alt={item.name} width={150} height={150} />
              )}
              <div>
                <h2 className="text-lg font-bold">{item.name}</h2>
                <p>{item.description}</p>
                <p className="text-sm text-gray-500">Created by: {item.created_by}</p>
                <p className="text-sm text-gray-500">Status: {item.cdt3d_status}</p>
                <p className="text-sm text-gray-500">Message: {item.cdt3d_msg}</p>
                <p className="text-sm text-gray-500">
                  url: <a href={`${process.env.NEXT_PUBLIC_R2_BASE}/${item.id}`}>{item.id}</a>
                </p>
              </div>
            </div>
          </ListGroupItem>
