...
export default function ServiceWorkerDemoPage() {
  ...
  const fetchCacheInfo = useCallback(async () => {
    ...
    try {
      ...
      if (result?.cacheInfo) {
        ...
        logMessage(
          `${result.cacheInfo.totalCaches}個のキャッシュが見つかりました`
        );
      } else {
        setCacheInfo(null);
  ...
  const handleAddDemoCache = useCallback(async () => {
    ...
    try {
      ...
      if (result?.success) {
            ...
            setTimeout(() => {
              setAnimatedItems((prev) => {
                ...
                if (updated[targetCacheIndex]) {
                  updated[targetCacheIndex].items = updated[
                    targetCacheIndex
        ...
        logMessage(
          `デモキャッシュアイテム追加失敗: ${result?.error || "不明なエラー"}`
        );
        setVisualFeedback({
          type: "error",
  ...
  }, [clearCache, cacheName, logMessage, fetchCacheInfo, cacheInfo]);
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">Service Worker 詳細デモ</h1>
      {/* タブパネル */}
      <div className="mb-6">
        <div className="border-b border-gray-200">
          <nav className="flex flex-wrap -mb-px">
            <button
              ...
              className={`mr-2 py-2 px-4 font-medium text-sm leading-5 rounded-t-lg ${
                activeTab === "basic"
      ...
      {activeTab === "basic" && (
        <div className="border rounded-lg p-4 mb-6 bg-white">
          ...
          <div className="mb-4">
            <h3 className="text-md font-medium mb-2">ステータス</h3>
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            <button
              ...
              disabled={!isRegistered}
              className="px-3 py-2 bg-green-500 text-white rounded hover:bg-green-600 disabled:bg-gray-300 text-sm"
            >
              更新チェック
            </button>
          </div>
          ...
          {registration && (
            <div className="mt-4 p-3 bg-gray-50 rounded border">
              <h3 className="text-md font-medium mb-2">登録情報</h3>
      ...
      {activeTab === "messaging" && (
        <div className="border rounded-lg p-4 mb-6 bg-white">
          <h2 className="text-xl font-semibold mb-4">メッセージング</h2>

          <div className="mb-4">
            <h3 className="text-md font-medium mb-2">メッセージ送信</h3>
            ...
            <p className="text-sm text-gray-500">
              ECHOタイプのメッセージを送信して応答を受け取ります
            </p>
          </div>

          <div className="mb-4">
            <h3 className="text-md font-medium mb-2">購読中のイベント</h3>
      ...
      {activeTab === "cache" && (
        <div className="border rounded-lg p-4 mb-6 bg-white">
          <h2 className="text-xl font-semibold mb-4">キャッシュ操作</h2>
          ...
          <div className="mb-4 border-t pt-4">
            <h3 className="text-md font-medium mb-2">キャッシュクリア</h3>
            <div className="flex mb-3">
              <input
                type="text"
                value={cacheName}
              ...
              <button
                ...
                disabled={
                  !isRegistered ||
                  (!cacheName && (!cacheInfo || cacheInfo.totalCaches === 0))
                }
              ...
              <datalist id="cache-suggestions">
                {cacheInfo?.details?.map(
                  (cache: CacheInfo["details"][number], index: number) => (
            ...
            <div className="p-3 bg-yellow-50 border border-yellow-200 rounded">
              ...
              <p className="text-xs text-gray-700">
                Service
                Workerのキャッシュは、オフライン対応やパフォーマンス向上のために使用されます。
      ...
      {activeTab === "advanced" && (
        <div className="border rounded-lg p-4 mb-6 bg-white">
          <h2 className="text-xl font-semibold mb-4">高度な操作</h2>
          ...
          <div className="mb-4">
            ...
            <p className="text-sm text-gray-500 mt-2">
              待機中のService
              Workerをアクティブ化します（ページがリロードされます）
            </p>
          </div>

          <div className="mb-4">
            <h3 className="text-md font-medium mb-2">手動更新</h3>
            <button
              onClick={() => {
          ...
          <div className="p-3 bg-yellow-50 border border-yellow-200 rounded">
            <p className="text-sm">
              ...
              これらの操作は開発時またはトラブルシューティング時に使用します
            </p>
          </div>
        </div>
      )}
      ...
      <div className="border rounded-lg p-4 bg-white">
        ...
        <div className="bg-gray-50 p-3 rounded h-64 overflow-y-auto font-mono text-sm">
          {logs.length === 0 ? (
...
const CacheStatusDisplay = ({
  ...
  if (!cacheInfo || cacheInfo.totalCaches === 0) {
    return (
      <div className="p-6 bg-gray-50 rounded border text-center">
        ...
        <p className="text-gray-500 mb-4">キャッシュが見つかりません</p>
      ...
      <div className="bg-gray-100 p-3 border-b">
        <div className="flex justify-between items-center">
          <div className="font-medium flex items-center">
            <span className="mr-2">
              合計 {cacheInfo.totalCaches} キャッシュ
            </span>
              ...
              <div className="mt-2 text-xs">
                ...
                <ul className="space-y-1">
                  {cache.urls.map((url, i) => (
                    <li
                      key={i}
                      className={`truncate text-gray-700 p-1 rounded ${
                        animatedItems[index]?.items[i]?.isNew
                          ? "bg-green-100"
                          : ""
                      }`}
                    >
                ...
                {cache.hasMore && (
                  <p className="text-gray-400 mt-1 italic">
                    他 {cache.size - 5} アイテムは省略されています...
                  </p>
                )}
              </div>
            ) : (
              <p className="text-xs text-gray-400 mt-1">アイテムがありません</p>
            )}
          </div>
        ))}
      </div>
