...
import { useWorkerJob } from "@/hooks/useWorkerJob";

/**
 * useWorkerJobの基本的な使用例
 *
 * このコンポーネントは:
...
const SimpleJobWorkerDemo = () => {
  ...
  // useWorkerJobフックの初期化（基本的な設定のみ）
  const { executeJob, isRunning, lastResult } = useWorkerJob({
    // ワーカースクリプトのパス
  ...
  return (
    <div className="flex flex-col pt-[120px] items-center justify-center">
      ...
      {result !== null && (
        <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded max-w-md">
          <p className="font-medium">
            計算結果:{" "}
            <span className="text-green-700">{result.toLocaleString()}</span>
          </p>
          {lastResult && (
            <p className="text-sm text-gray-500 mt-1">
              処理時間: {lastResult.duration.toFixed(0)}ms
            </p>
          )}
        </div>
      )}
      ...
      {/* 高機能ページへのリンク */}
      <div className="mt-6">
        <a href="/demo/jw" className="text-blue-600 hover:underline text-sm">
          詳細機能のデモページへ
        </a>
        <p className="text-xs text-gray-500 mt-1">
          ※詳細デモでは、useWorkerJobフックの全機能を体験できます。
          <br />
