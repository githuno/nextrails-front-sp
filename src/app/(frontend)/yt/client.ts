...
class YouTubeClient {
  ...
  async search(params: SearchParams): Promise<SearchResponse> {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `検索に失敗しました: ${response.status} ${response.statusText}`
        );
      }
  ...
  async getVideoDetails(videoIds: string[]): Promise<VideoDetail[]> {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `動画情報の取得に失敗しました: ${response.status} ${response.statusText}`
        );
      }
  ...
  async getChannelDetails(channelIds: string[]): Promise<ChannelDetail[]> {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `チャンネル情報の取得に失敗しました: ${response.status} ${response.statusText}`
        );
      }
  ...
  async searchChannelVideos(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `チャンネル動画の検索に失敗しました: ${response.status} ${response.statusText}`
        );
      }
