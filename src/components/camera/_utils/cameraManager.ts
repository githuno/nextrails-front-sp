...
class CameraManager {
  ...
  constructor(setState: React.Dispatch<React.SetStateAction<CameraState>>, config: CameraConfig = {}) {
  ...
  public async startQrScan(): Promise<void> {
    if (!this.videoElement || !this.canvasElement) {
      throw new Error("video or canvas element is not set. please setupVideo first")
    }
  ...
  public async capture(onComplete: (url: string | null) => void): Promise<void> {
  ...
  // 不要
  public async setPreviewElements(videoElement: HTMLVideoElement, canvasElement: HTMLCanvasElement): Promise<void> {
    this.videoElement = videoElement
    this.canvasElement = canvasElement
  }
...
export { CameraManager, type CameraCallbacks, type CameraConfig, type CameraState }
