...
const useSyncLatests = () => {
  ...
  const pullImagesets = useCallback(
    async ({ params, excludeSetName }: { params?: string; excludeSetName?: string }): Promise<Imageset[]> => {
      if (!cloud.storage) return []

      try {
        // 1. バックエンドからimagesets情報を取得
        const json = await apiFetch<Imageset[]>(`/imagesets${params ? `?${params}` : ""}`)
        if (!json) return []
            ...
            const files = await Promise.all(
              imageset.files.map(async (file: File) => {
                ...
                const blobs: (Blob | null)[] =
                  (await cloud.provider?.download({ keys: [file.key ?? ""] }).catch((error: Error): (Blob | null)[] => {
        ...
        // nullは除外して返す
        return cloudImagesets.filter((imageset): imageset is Imageset => imageset !== null)
      } catch (error) {
      ...
      const syncPromises = latestImagesets
        ...
        .map(async (store) => {
          ...
          try {
            ...
            console.error(`Error syncing store ${store} with cloud files:`, error)
          }
        })
