...
const useSyncImageset = () => {
  ...
  const pullFiles = useCallback(
    async ({ setName, options }: { setName: string; options?: { params: string } }): Promise<File[]> => {
      ...
      try {
        // バックエンドからファイル情報を取得
        const json = await apiFetch<File[]>(`/files?name=${setName}${options ? `&${options.params}` : ""}`)
        if (!json) return []

        console.log("⚡️ json:", json)

        // クラウドストレージからblobを取得（対象は未削除のみ）
        const keys = json.filter((file) => !file.deletedAt).map((file) => file.key as string)
        const blobs = await cloud.provider?.download({ keys }).catch((error: Error) => {
        ...
        const files = json.map((file) => ({
          ...
          idbId:
            file.key
              ...
              .pop()
              ?.replace(/\.[^/.]+$/, "") || "",
          blob: !file.deletedAt && blobs ? blobs[keys.indexOf(file.key!)] || null : null,
        }))
  ...
  const postFile = useCallback(
    async (imagesetName: string, file: File): Promise<File | undefined> => {
      ...
      try {
        ...
        const updatedFile = await apiFetch<File>(`/files?name=${imagesetName}`, {
          method: "POST",
  ...
  const putFile = useCallback(async (imagesetName: string, file: File): Promise<File | undefined> => {
    setIsSyncing((prev) => [...prev, file.idbId])
    try {
      if (!file.id || !file.version || !file.createdAt || !file.updatedAt) return
      const updatedFile = await apiFetch<File>(`/files/${file.id}?name=${imagesetName}`, {
        method: "PUT",
  ...
  const deleteFile = useCallback(async (imagesetName: string, file: File): Promise<void> => {
    setIsSyncing((prev) => [...prev, file.idbId])
    try {
      if (!file.id || !file.version || !file.updatedAt || !file.deletedAt) throw new Error("Invalid file data")
  ...
  const syncPullFiles = useCallback(
    async ({ setName, options }: { setName: string; options?: { params: string } }): Promise<void> => {
          ...
          prev.name === set.name
            ? {
                ...
                files: file.deletedAt
                  ...
                  : prev.files.map((f) => (f.idbId === newFile.idbId ? newFile : f)),
              }
            : prev,
        )
      } catch (error) {
