...
export interface ServiceWorkerContextType extends ServiceWorkerState {
  // コア機能のAPIをそのまま継承
  register: (options?: ServiceWorkerOptions) => Promise<ServiceWorkerRegistration | null>
  unregister: () => Promise<boolean>
  update: () => Promise<boolean>
  sendMessage: <T = any, R = any>(message: ServiceWorkerMessage<T>) => Promise<R | null>
  subscribeToMessage: <T = any>(type: ServiceWorkerMessageType, callback: (data: T) => void) => () => void
...
const ServiceWorkerContext = createContext<ServiceWorkerContextType>(defaultContext)
...
export function ServiceWorkerProvider({
  ...
  const [state, setState] = useState<ServiceWorkerState>(serviceWorker.getState())
  ...
  useEffect(() => {
    ...
    // serviceWorkerの初期化
    serviceWorker.init(options).catch((e) => console.error("Service Worker初期化エラー:", e))
...
export function useServiceWorker(): ServiceWorkerContextType {
  ...
  // 現在のステート
  const [directState, setDirectState] = useState<ServiceWorkerState>(serviceWorker.getState())
  ...
  useEffect(() => {
    ...
    // serviceWorkerの初期化
    serviceWorker.init().catch((e) => console.error("Service Worker初期化エラー:", e))
...
export type {
  ...
  ServiceWorkerState,
} from "@/hooks/useWorker/utils/serviceWorker"
 ...
 * });
 */
export function createServiceWorkerContext(name: string, defaultOptions: ServiceWorkerOptions = {}) {
  // 固有の名前付きコンテキストを作成
  const CustomServiceWorkerContext = createContext<ServiceWorkerContextType>(defaultContext)
  ...
  function CustomServiceWorkerProvider({
    ...
    useEffect(() => {
      ...
      // serviceWorkerの初期化
      serviceWorker.init(mergedOptions).catch((e) => console.error(`${name} Service Worker初期化エラー:`, e))
    ...
    const subscribeToContextMessage = useCallback(
      <T = any,>(type: ServiceWorkerMessageType, callback: (data: T) => void) => {
        // コンテキスト固有のイベントとグローバルイベントの両方を購読
        const contextUnsubscribe = pubSub.on(`${eventPrefix}${type}` as any, callback)
        const globalUnsubscribe = pubSub.on(`sw:${type}` as any, (data: any) => {
    ...
    const contextValue = {
      ...
      checkForUpdates: serviceWorker.checkForUpdates,
      skipWaiting: serviceWorker.skipWaiting,
      clearCache: (cacheName?: string) => serviceWorker.clearCache(cacheName ? `${name}-${cacheName}` : undefined),

      // React固有の追加機能
      showUpdatePrompt,
    }

    return <CustomServiceWorkerContext.Provider value={contextValue}>{children}</CustomServiceWorkerContext.Provider>
  }
  ...
  function useCustomServiceWorker(): ServiceWorkerContextType {
    ...
    if (context === defaultContext) {
      console.warn(`${name}ServiceWorkerProvider が見つかりません。デフォルト実装を使用します。`)
