...
import { JobOptions, JobResult } from "./utils/job"

// useWebWorkerフックのオプション
 ...
 */
export function useWebWorker<ResultType = any, PayloadType = any>(options: UseWebWorkerOptions) {
  // デフォルト値の設定
  const {
    scriptUrl,
    type = "classic",
  ...
  const [autoTerminate, setAutoTerminate] = useState<boolean>(terminateAfterJob)
  ...
  const getWorker = useCallback(() => {
    ...
    // 既存Workerが古すぎる場合は破棄
    if (workerRef.current && now - creationTimeRef.current > maxWorkerLifetime) {
  ...
  // クリーンアップ処理
  useEffect(() => {
    // 現在のabortControllersRefの値をキャプチャ
  ...
  const executeJob = useCallback(
    async (jobOptions: JobOptions<PayloadType>): Promise<JobResult<ResultType>> => {
      const {
        payload,
      ...
      while (currentAttempt <= retries) {
        ...
        try {
          // 実行状態を更新
          setIsRunning(true)
          log(`Starting job ${jobId}, attempt ${currentAttempt}/${retries + 1}`)
          ...
          const result = await new Promise<ResultType>((resolve, reject) => {
            ...
            if (enableProgress) {
              ...
              progressHandlersRef.current.set(jobId, progressCallback)
            }
              ...
              if (type === "RESULT") {
                ...
                if (jobOptions.onProgress) {
                  jobOptions.onProgress(responsePayload?.percent || 0, {
                    jobId,
                    status: "running",
                    progress: responsePayload?.percent || 0,
                    startTime: Date.now(),
            ...
            const cleanup = () => {
              ...
              if (progressCallback) {
                progressHandlersRef.current.delete(jobId)
              }
          ...
          log(`Job ${jobId} completed in ${duration.toFixed(1)}ms`)
          ...
          if (autoTerminate) {
            log(`Auto-terminating worker after job ${jobId} (terminateAfterJob=${autoTerminate})`)
            terminateWorker()
          }
          ...
          log(`Job ${jobId} failed:`, lastError)

          // 最後のリトライでなければ再試行
          if (currentAttempt <= retries) {
            log(`Retrying job ${jobId} in ${retryDelay}ms (attempt ${currentAttempt}/${retries})`)
    ...
    [getWorker, terminateWorker, globalTimeout, log, debug, autoTerminate],
  )
  ...
  const sendDirectMessage = useCallback(
    async <T = any, R = any>(message: T, timeout = 5000, transferables?: Transferable[]): Promise<R | null> => {
      const worker = getWorker()
