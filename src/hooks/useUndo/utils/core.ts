...
import { MemoryUsage, OperationRecord, UndoableAction, UndoStack, UndoStackOptions, UndoStackState } from "./types"
...
export function createUndoStack<T>(options: UndoStackOptions = {}): UndoStack<T> {
  ...
  const totalMemoryLimitBytes = maxMemorySize * 1024 * 1024 // MB → バイト
  ...
  const garbageCollect = () => {
    ...
    if (debug) {
      ...
      logger.log(`解放されたメモリ: ${(freedBytes / 1024 / 1024).toFixed(2)} MB`)
      logger.log(`現在のメモリ使用量: ${(memoryUsage.estimatedBytes / 1024 / 1024).toFixed(2)} MB`)
  ...
  const setupGCTimer = () => {
    if (perfMonitorInstance && performanceMonitoring) {
      ...
      perfMonitorInstance.scheduleGC("undoStackGC", garbageCollect, adjustedInterval)
  ...
  const isDiffAction = (action: UndoableAction<T>): boolean => {
    return !!action.isMutation && Array.isArray(action.targetPaths) && action.targetPaths.length > 0
  }
  ...
  const createSelectiveClone = (() => {
    ...
    return (state: T): T => {
      // 参照が同じなら前回の結果を返す
      if (state === cachedState && (!selectivePaths || selectivePaths === cachedPaths)) {
        return cachedResult
      }
      ...
      cachedState = state
      cachedPaths = selectivePaths
      cachedResult = objUt.selectiveDeepClone(state as Record<string, any>, selectivePaths) as T
      return cachedResult
    }
  })()
  ...
  const enforceMemoryLimits = () => {
    ...
    if (memoryUsage.estimatedBytes > totalMemoryLimitBytes) {
      ...
      if (pluginEventMap.memoryWarning) {
        emitPluginEvent("memoryWarning", sortedPlugins, logger, pluginEventMap, memoryUsage, stack)
      }
    }
  }
  ...
  const stack: UndoStack<T> = {
      ...
      const { result, duration } = withPerformanceTracking(
        ...
        () => {
          ...
          if (pluginEventMap.actionPush) {
            emitPluginEvent("actionPush", sortedPlugins, logger, pluginEventMap, action, stack)
          }
    ...
    createDiffAction(prevState, nextState, label) {
      ...
      try {
        const { pathDiffs, diff, reverseDiff } = objUt.analyzeDiff(prevState, nextState)
        ...
        logger.warn(`差分計算に失敗しました: ${err.message}。完全な状態を使用します。`)
        return this.createAction(
          () => nextState,
          () => prevState,
      ...
      const { result } = withPerformanceTracking(
        ...
        () => {
          ...
          if (isDiffAction(finalAction)) {
            logger.log(`差分ベースのアクションを実行: ${finalAction.targetPaths?.length || 0}のパスが影響を受けます`)
          }
          ...
          // プラグインの実行
          if (pluginEventMap.actionPush) {
            emitPluginEvent("actionPush", sortedPlugins, logger, pluginEventMap, finalAction, stack)
          }
      ...
      const { result } = withPerformanceTracking(
        ...
        () => {
          ...
          if (pluginEventMap.undo) {
            emitPluginEvent("undo", sortedPlugins, logger, pluginEventMap, action, stack)
          }
          ...
          if (undoResult && typeof undoResult === "object") {
            ...
            lastState = isDiffAction(action) ? objUt.deepClone(undoResult) : undoResult
      ...
      const { result } = withPerformanceTracking(
        ...
        () => {
          ...
          if (pluginEventMap.redo) {
            emitPluginEvent("redo", sortedPlugins, logger, pluginEventMap, action, stack)
          }
          ...
          if (redoResult && typeof redoResult === "object") {
            lastState = isDiffAction(action) ? objUt.deepClone(redoResult) : redoResult
          } else {
    ...
    batch(actions, label) {
      ...
      const { result } = withPerformanceTracking(
        "push",
        () => {
          logger.log(`バッチアクション作成: ${label || "batch"} (${actions.length}件)`)
        ...
        () => {
          ...
          if (past.length > 0 || future.length > 0) {
            ...
            if (pluginEventMap.clear) {
              emitPluginEvent("clear", sortedPlugins, logger, pluginEventMap, stack)
            }
