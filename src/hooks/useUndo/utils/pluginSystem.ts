import { PluginEventHandlers, PluginEventName, UndoDebugger, UndoStackPlugin } from "./types"

/**
 * プラグインの依存関係を検証
 */
export function validatePluginDependencies<T>(plugins: UndoStackPlugin<T>[], logger: UndoDebugger): boolean {
 ...
 * 依存関係に基づいてプラグインをソート
 */
export function sortPluginsByDependencyOrder<T>(plugins: UndoStackPlugin<T>[]): UndoStackPlugin<T>[] {
 ...
 */
export function hasPluginForEvent<T>(event: PluginEventName, plugins: UndoStackPlugin<T>[]): boolean {
