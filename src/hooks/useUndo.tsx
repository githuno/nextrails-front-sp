...
import { createActionTrackerPlugin } from "@/utils/undo/plugins";
import { objUt } from "@/utils/objectUtils";
import { perfMonitor } from "@/utils/performanceMonitor"; // パフォーマンスモニターをインポート
...
interface UseUndoResult<T> {
  ...
  // バッチ処理とアクション作成
  batch: (
    fnOrActions: (() => void) | UndoableAction<T>[],
    label?: string
  ) => void;
  createAction: <R>(
    doFn: () => R,
...
function useEfficientUndo<T>(
  ...
  const perfMonitorInstance = stackOptions.performanceMonitoring
    ? perfMonitor
    : undefined;
  ...
  const undoStack = useMemo(
    ...
    [
      // 主要な設定オプションのシリアライズ化
      JSON.stringify({
        debug: stackOptions.debug,
        compress: stackOptions.compress,
        memoryEfficient: stackOptions.memoryEfficient,
        historyLimit,
        memoryBasedLimit: stackOptions.memoryBasedLimit,
        maxMemorySize: stackOptions.maxMemorySize,
      }),
      // プラグイン配列の参照または長さが変わった場合
      plugins,
    ]
  );
  ...
  // 履歴状態も単純に初期化
  const [historyState, setHistoryState] = useState<UndoStackState>(
    undoStack.state
  );
        ...
        if (isBatchingRef.current) {
          if (!options?.silent) {
            ...
            const action = useDiff
              ? undoStackRef.current.createDiffAction(
                  prevState,
                  computedState,
                  effectiveLabel
                )
   ...
   * 差分ベースのアクションを作成
   */
  const createDiffAction = useCallback(
    (prevState: T, nextState: T, label?: string): UndoableAction<T> => {
      ...
      if (typeof fnOrActions === "function") {
        ...
        try {
          if (fnOrActions.length > 0) {
            ...
            const result = undoStackRef.current.batch(
              fnOrActions,
              label || "batch"
            );
            if (result !== undefined) {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (
        (redoKeys.includes(e.key) || (e.key === "z" && e.shiftKey)) &&
  ...
  useEffect(() => {
    ...
    stackOptions.debug,
    stackOptions.performanceMonitoring,
    perfMonitorInstance,
  ]);
